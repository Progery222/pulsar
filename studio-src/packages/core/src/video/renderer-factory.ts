...
export function isWebGPUSupported(): boolean {
  ...
  // Встроенный режим: по умолчанию Canvas2D — на части драйверов WebGPU
  // инициализируется, но рисует чёрным (как WebGL→белый). Включить: window.__ENABLE_WEBGPU__ = true.
  if (typeof window !== "undefined" && (window as unknown as { __ENABLE_WEBGPU__?: boolean }).__ENABLE_WEBGPU__ !== true) {
    return false;
  }

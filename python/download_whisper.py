...
BASE = "https://hf-mirror.com/Systran/faster-whisper-{m}/resolve/main/{f}"
...
def main():
    ...
    for f in FILES:
        ...
        if os.path.exists(dest) and os.path.getsize(dest) > 0:
            _p("Уже скачано: %s" % f)
            continue
        url = BASE.format(m=args.model, f=f)
        ok = False
        for attempt in range(40):
            try:
                ...
                _p("Повтор %s (попытка %d): %s — пауза %dс" % (f, attempt + 1, str(e)[:100], wait))
                time.sleep(wait)
        if not ok:
